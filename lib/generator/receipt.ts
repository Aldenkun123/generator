...
// Word-wrap helper
function wrap(ctx: ReturnType<typeof createCanvas>["getContext"], text: string, maxW: number): string[] {
...
export async function renderReceiptImage(format: "png" | "jpeg" = "png") {
  ...
  for (let i = 0; i < infoRows.length; i++) {
    ...
    ctx.font = "12px Arial, sans-serif"; ctx.fillStyle = "#666"; ctx.textAlign = "left"
    ctx.fillText(label, PAD + 12, y)
    ctx.font = "bold 12px Arial, sans-serif"; ctx.fillStyle = "#111"; ctx.textAlign = "right"
  ...
  for (const item of data.items) {
    ctx.font = "bold 13px Arial, sans-serif"; ctx.fillStyle = "#111"; ctx.textAlign = "left"
    for (const line of wrap(ctx, item.name, W - PAD * 2)) { ctx.fillText(line, PAD, y); y += 18 }
    ctx.font = "11px Arial, sans-serif"; ctx.fillStyle = "#888"; ctx.textAlign = "left"
  ...
  for (const [label, value, color] of totRows) {
    ctx.font = "12px Arial, sans-serif"; ctx.fillStyle = "#555"; ctx.textAlign = "left"
    ctx.fillText(label, PAD + 12, y)
    ctx.font = "12px Arial, sans-serif"; ctx.fillStyle = color; ctx.textAlign = "right"
  ...
  ctx.font = "bold 15px Arial, sans-serif"; ctx.fillStyle = "#fff"; ctx.textAlign = "left"
  ...
  // ── PAYMENT INFO ──
  ctx.font = "12px Arial, sans-serif"; ctx.fillStyle = "#555"; ctx.textAlign = "center"
  ...
  ctx.font = "bold 12px Arial, sans-serif"; ctx.fillStyle = "#111"
  ...
  // ── FOOTER ──
  ctx.font = "10px Arial, sans-serif"; ctx.fillStyle = "#aaa"; ctx.textAlign = "center"
  ...
  ctx.font = "bold 36px Arial, sans-serif"; ctx.textAlign = "center"
